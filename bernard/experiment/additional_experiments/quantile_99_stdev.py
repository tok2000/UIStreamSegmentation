...
def segmentation(dataframe, k, activity, time_stamp):
    ...
    for i in range(j, len(dataframe) - 1):
        ...
        if not math.isnan(x_0):
            ...
            pred['TOK_MPTAP'] = pairs[pair][1] - (mptap_mean + mptap_st_dev)
            pred['TOK_MPTAP_is_cut'] = pred['TOK_MPTAP'] > 0

            buffer.append(pairs[pair][1])
            if len(buffer) > 100:
                buffer.pop(0)

            if pairs[pair][1] < np.quantile(buffer, 0.99):
                mptap_mean_new = (pairs[pair][1] + c * mptap_mean) / (c + 1)
                mptap_var = ((c - 1) * mptap_var + c * ((mptap_mean - mptap_mean_new) ** 2) + (
                        (pairs[pair][1] - mptap_mean_new) ** 2)) / c
    ...
    if second_last is not None:
        ...
        second_last['TOK_MPTAP'] = second_last[time_diff] - (mptap_mean + mptap_st_dev)
...
def evaluate(logs):
    with pd.ExcelWriter('../results/additional_results/' + parameter + '.xlsx') as mptap_writer:
        for log_name in logs:
            ...
            # df.to_csv('./results/results_complete/configuration/' + log_name + '_segmented_' + parameter + '.csv', sep=',', index=False)
