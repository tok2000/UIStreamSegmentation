...
def evaluate(logs):
    with pd.ExcelWriter('./results/results_complete/different_k/' + parameter + '.xlsx') as mptap_writer:
...
parameter = 'bernard_k_' + str(k)

segmented_logs['student'] = leno_log('StudentRecord', k)
segmented_logs['reimb'] = leno_log('Reimbursement', k)
